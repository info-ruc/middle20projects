...
<article class="page-container">
	<form action="<%=request.getContextPath() %>/editRoom" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>房间号：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="hidden" name="roomId" value="${room.roomId }">
				<input type="text" class="input-text" value="${room.roomNumber }" readonly="readonly" placeholder="" id="roomNumber" name="roomNumber">
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${room.roomArea }" placeholder="" id="roomArea" name="roomArea">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>房间价格：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${room.roomPrice }" placeholder="" id="roomPrice" name="roomPrice">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3">房间描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="roomDesc" cols="" rows=""  class="textarea"  placeholder="说点什么...最少输入10个字符" >${room.roomDesc }</textarea>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" size="1" name="roomTypeId">
					<c:forEach items="${roomTypeList }" var="rl">
						<option value="${rl.typeId }" <c:if test="${rl.typeId == room.roomTypeId }">selected="selected"</c:if>>${rl.typeName }</option>
					</c:forEach>
				</select>
				</span>
			</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal">
				<div class="radio-box">
					<input name="specail" type="checkbox"  value="1、空调" <c:if test="${room.specail1 == '1、空调'  }">checked="checked" </c:if> >
					<label for="sex-1">1、空调</label>
				</div>
				<div class="radio-box">
					<input type="checkbox" name="specail" value="2、WIFI" <c:if test="${room.specail2 == '2、WIFI'  }">checked="checked" </c:if>>
					<label for="sex-2">2、WIFI</label>
				</div>
				<div class="radio-box">
					<input type="checkbox" name="specail" value="3、24小时热水" <c:if test="${room.specail3 == '3、24小时热水'  }">checked="checked" </c:if>>
					<label for="sex-3">3、24小时热水</label>
				</div>
				<div class="radio-box">
					<input type="checkbox" name="specail" value="4、可煮饭" <c:if test="${room.specail4 == '4、可煮饭'  }">checked="checked" </c:if>> 
					<label for="sex-3">4、可煮饭</label>
				</div>
				<div class="radio-box">
					<input type="checkbox"  name="specail" value="5、大床房" <c:if test="${room.specail5 == '5、大床房'  }">checked="checked" </c:if>>
